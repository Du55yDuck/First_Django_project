...
AUTHENTICATION_BACKENDS = ['django.contrib.auth.backends.ModelBackend',  # Коллекция, содержит класс ModelBackend для
...
EMAIL_BACKEND = "django.core.mail.backends.smtp.EmailBackend"  # Отправка писем через smtp-протокол почтового сервера

EMAIL_HOST = "smtp..."  # адрес почтового сервера
EMAIL_PORT = 465
EMAIL_HOST_USER = "Эл. почта"  # почта-отправитель
EMAIL_HOST_PASSWORD = "Ваш уникальный пароль"  # уникальный пароль, получен 1 раз для связки сервера с приложением
EMAIL_USE_SSL = True  # защита соединения
