...
@login_required  # декоратор, ограничивает доступ не авторизованным пользователям + можно указать адрес перенаправления
...
class AddPage(PermissionRequiredMixin, LoginRequiredMixin, DataMixin, CreateView):  # Class типа (CreateView) исп-т спец
    ...
    def form_valid(self, form):  # метод связывающий автора со статьей(form - проверенная и заполненная форма)
        w = form.save(commit=False)  # объект новой записи для БД, но не сохраняет ее
        w.author = self.request.user  # присвоение объекта user(текущий пользователь) атрибуту author(связывает их)
