...
class AddPage(CreateView):  # Класс-аналог класса(View) и ф-ии выше. Class типа (CreateView) использует спец переменную
    ...
    # model = Women - можно явно указать связь с моделью, которая должна содержать def get_absolute_url
    # fields = ['title', 'slug', 'content', 'is_published', 'cat'] - также указать поля для обязательного заполнения.
    ...
    #         return super().form_valid(form) - Метод был в class AddPage(FormView). Для (CreateView) не требуется.


class UpdatePage(UpdateView):  # Класс представления типа (UpdateView) для обновления статей
    ...
    extra_context = {  # поля в заголовке страницы
        'menu': menu,
