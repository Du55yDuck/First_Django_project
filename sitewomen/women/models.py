...
class Women(models.Model):  # наш класс-модель с полями для таблицы
    ...
    # def save(self, *args, **kwargs):  # Спец метод для сохранения с параметрами.(имеется у каждой модели)
        # self.slug = slugify(self.title)  # применить к текущей записи и сформировать slug по заголовку
        # super().save(*args, **kwargs)  # вызвать save() из базового класса
